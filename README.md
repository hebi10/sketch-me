...
- 관리자는 그림 한 장을 담은 1:1 이미지와 선정된 그림을 모은 BEST 4 이미지를 제작할 수 있습니다.
- 한 장 이미지와 BEST 이미지는 제목을 각각 저장하며, 한 장 이미지는 그린 사람 이름으로 공개 가능한 그림을 검색해 선택합니다.
- 워터마크 제거 권한은 두 제작 유형에 공통으로 적용됩니다. 재진입하면 저장한 제목과 권한은 복원되고, 그림 선택과 디자인은 기본 상태로 시작합니다.
...
- 운영자 차단은 이후 공개 응답을 막지만, 이미 내려받았거나 외부에 저장된 공유 PNG는 회수할 수 없습니다.
...
결제 화면과 결제 통계는 페이앱 주문 상태를 기준으로 표시합니다. 관리자는 성공한 페이앱 결제의 전체 취소를 요청할 수 있으며, 취소 후 이미 적용된 상품 혜택은 자동으로 회수되지 않으므로 운영 정책에 따라 별도로 확인합니다. 운영자 차단은 이후 공개 페이지, 제출, 이미지 응답을 막지만 이미 다운로드했거나 외부에 저장한 공유 PNG까지 회수하지는 못합니다.

## 페이앱 실결제 설정

페이앱 판매자 가입과 계약·심사를 완료한 뒤 서버 런타임에 아래 비밀 변수를 설정합니다. 실제 값은 `.env.local`, 저장소, 로그 또는 클라이언트 코드에 기록하지 않습니다.

- `PAYAPP_USER_ID`: 페이앱 판매자 아이디
- `PAYAPP_LINK_KEY`: 페이앱 연동 KEY
- `PAYAPP_LINK_VALUE`: 페이앱 연동 VALUE
- `NEXT_PUBLIC_APP_URL`: 외부에서 접근 가능한 HTTPS 배포 Origin

결제 요청은 서버에서 `https://api.payapp.kr/oapi/apiLoad.html`로 전송합니다. 완료 처리는 `POST /api/payments/payapp/feedback`, 결제창 복귀는 `GET|POST /api/payments/payapp/return`으로 수신합니다. 복귀 URL만으로는 혜택을 적용하지 않으며, 판매자 아이디·연동 KEY·VALUE·페이앱 주문번호·금액이 모두 일치하는 완료 통보만 Firestore 트랜잭션으로 한 번 적용합니다.

판매자 화면의 연동 KEY와 VALUE를 서버 비밀 변수에 등록합니다. 앱이 결제 요청마다 `feedbackurl`을 전달하므로 판매자 화면의 공통 통보 URL은 비워 둡니다. 두 설정을 함께 사용하면 같은 결제 통보가 중복될 수 있습니다.

관리자 결제 목록의 전체 취소는 페이앱 즉시 취소만 요청합니다. 이미 정산된 거래는 페이앱 판매자 사이트에서 취소를 요청하고 안내된 반환금 계좌와 금액을 확인해 입금한 뒤, 승인 취소 통보가 반영되었는지 확인합니다. 취소 후 이미 적용된 상품 혜택은 자동 회수되지 않습니다.

운영 전에는 페이앱 판매자 화면에서 결제수단과 정산 상태를 확인하고 다음 순서로 소액 승인·전체 취소를 한 번씩 검증합니다.

1. HTTPS 운영 주소와 세 비밀 변수를 설정하고 새 빌드를 배포합니다.
2. 1,000원 상품으로 결제하여 페이앱 결제창, 복귀 화면, 관리자 결제 목록의 성공 상태를 확인합니다.
3. 같은 완료 통보가 반복되어도 혜택이 한 번만 적용되는지 확인합니다.
4. 정산 전 거래는 관리자 결제 목록에서 전체 취소를 요청하고 페이앱과 앱의 취소 상태를 확인합니다.
5. 결제용 휴대전화번호 원문과 연동 비밀값이 로그·Firestore·브라우저 응답에 남지 않는지 확인합니다.
