...
function parseHttpLoopbackUrl(value: string | undefined) {
...
function parseBareLoopbackAddress(value: string | undefined): LoopbackAddress | null {
  ...
  const url = parseHttpLoopbackUrl(`http://${value}`);
...
function parseStorageAdminAddress(value: string | undefined): LoopbackAddress | null {
  const url = parseHttpLoopbackUrl(value);
  if (!url || !url.port) return null;
  return {
    host: url.hostname.replace(/^\[|\]$/g, '').toLowerCase(),
    port: Number(url.port),
...
export function resolvePlaywrightBaseUrl(value: string | undefined) {
  const url = parseHttpLoopbackUrl(value ?? 'http://127.0.0.1:3000');
  if (!url) {
...
export function getE2EServerReadinessState(
  ...
  try {
    ...
    const allowedOrigin = resolvePlaywrightBaseUrl(environment.ADMIN_ALLOWED_ORIGIN);
    const resolvedRequestOrigin = resolvePlaywrightBaseUrl(requestOrigin);
    ...
    if (
      ...
      || allowedOrigin !== resolvedRequestOrigin
    ) {
      return 'unavailable';
    }
