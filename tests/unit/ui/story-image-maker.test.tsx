...
describe('StoryImageMaker', () => {
  ...
  it('기본 저장 이미지에 생성된 스캐치북 워터마크를 합성한다', async () => {
    render(<StoryImageMaker backgroundImage="/story/background.webp" drawings={[]} name="내 이름" publicUrl="/s/book-1" watermarkFree={false} />);
  ...
  it('워터마크 제거가 적용되면 저장 이미지에 워터마크를 넣지 않는다', async () => {
    render(<StoryImageMaker backgroundImage="/story/background.webp" drawings={[]} name="내 이름" publicUrl="/s/book-1" watermarkFree />);
    fireEvent.click(screen.getByRole('button', { name: 'PNG로 저장하기' }));
