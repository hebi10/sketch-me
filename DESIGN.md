...
친구 그림과 BEST 카드는 320px 이상에서 2열로 유지하며 319px 이하에서만 1열로 축소한다. 편집 캔버스는 3:4 비율을 유지하고 모바일 세로 흐름에서 도구와 제출 폼이 이어진다.
...
그림은 3:4 비율의 흰 종이 카드 안에서 `contain`으로 전부 보인다. 이름과 시간은 같은 줄에 배치하고 긴 이름은 말줄임 처리한다. BEST 순위는 카드 왼쪽 위의 작은 블루 배지로 표시한다.
